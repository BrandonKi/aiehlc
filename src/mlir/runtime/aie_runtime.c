...
/* [exp47] localize the ~99.85% of bdcfg that is NOT descinit/writebd: bracket the
 * mid region (post-DescInit -> pre-WriteBd: Set* struct-fills + ooo printf) and the
 * tail region (post-WriteBd -> return: BD tracking). If mid+tail is still tiny the
 * cost is preemption/blocked wall-time, not CPU work in the function. */
...
/* [exp48] split the dominant bdcfg `mid` region (exp47: 99.9%) into the 3 XAie calls that run on
 * every path — GetTileTypefromLoc (gtt), DmaSet{AddrLen,MultiDimAddr} (saddr), DmaEnableBd (en).
 * Whatever the residual (mid - gtt - saddr - en) is = the Set{Lock,NextBd,Pkt,Ooo}+printf block. */
 ...
 * printfs are already AIEHLC_LOG-gated, so it is genuine HW work: 16x (CoreDisable + CoreReset +
...
void __Runtime_free(void *ptr) {
    ...
    if (AIEHLC_LOG_ENABLED() && !g_dumped_done) {
        g_dumped_done = 1;
...
AieRC __Runtime_perfcnt_setup(XAie_DevInst *dev, XAie_LocType tile, uint8_t counter_id, XAie_Events event) {
    ...
    if (rc != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] perfcnt_setup: PerfCounterSet failed tile(%u,%u) "
                          "counter=%u rc=%d\n",
    ...
    if (rc != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] perfcnt_setup: PerfCounterControlSet failed "
...
AieRC __Runtime_perfcnt_read(XAie_DevInst *dev, XAie_LocType tile, uint8_t counter_id, uint32_t *value) {
    ...
    if (rc != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] perfcnt_read: PerfCounterGet failed tile(%u,%u) "
                          "counter=%u rc=%d\n",
                          (unsigned)tile.Col, (unsigned)tile.Row, (unsigned)counter_id, (int)rc));
    } else {
...
AieRC __Runtime_perfcnt_setup_mm2s_bd_finished_partition(XAie_DevInst *dev, uint8_t start_col, uint8_t end_col,
    ...
    for (uint8_t col = start_col; col <= end_col; col++) {
        for (uint8_t row = start_row; row <= end_row; row++) {
            ...
            if (rc != XAIE_OK) {
                AIEHLC_LOG(printf("[aie_runtime] perfcnt_partition: FAILED at tile(%u,%u) "
                                  "rc=%d\n",
...
AieRC __Runtime_device_teardown(XAie_DevInst *dev) {
    ...
    if (AIE_DEBUG_HAS_FLAG(g_runtime_debug_level, AIE_DEBUG_FLAG_DISABLE_PARTITIONTEARDOWN)) {
        AIEHLC_LOG(printf("[aie_runtime] device_teardown SKIPPED (DISABLE_PARTITIONTEARDOWN flag set)\n"));
        return XAIE_OK;
    }
...
XAie_DevInst *__Runtime_explicit_init(void) {
    ...
    if (RC != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] explicit_init CfgInitialize failed: %d\n", (int)RC));
        free(dev);
        return NULL;
    }
    ...
    if (RC != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] explicit_init partition/pm failed: %d\n", (int)RC));
        free(dev);
        return NULL;
    }
...
XAie_DevInst *__Runtime_explicit_init_partition(int startCol, int numCols) {
    ...
    if (RC != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] explicit_init_partition SetupPartitionConfig failed: %d\n", (int)RC));
        free(dev);
        return NULL;
    }
    ...
    if (RC != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] explicit_init_partition CfgInitialize failed: %d\n", (int)RC));
        free(dev);
        return NULL;
    }
    ...
    if (RC != XAIE_OK) {
        AIEHLC_LOG(printf("[aie_runtime] explicit_init_partition partition/pm failed: %d\n", (int)RC));
        free(dev);
        return NULL;
    }
...
XAie_DmaDesc __Runtime_dma_bd_config(XAie_DevInst *dev, XAie_LocType tile, void *buffer, int32_t bd_id, int32_t len,
    ...
    g_bd_init_n++;
    unsigned long long __md0 = __rt_pmccntr(); /* [exp47] mid start */
    unsigned long long __gt0 = __rt_pmccntr(); /* [exp48] */
    uint8_t tile_type = XAie_GetTileTypefromLoc(dev, tile);
    g_bd_gtt_cyc += (__rt_pmccntr() - __gt0); /* [exp48] */
    g_bd_gtt_n++;
    ...
    unsigned long long __sa0 = __rt_pmccntr(); /* [exp48] */
    XAie_DmaSetAddrLen(&DmaInst, dma_addr, (uint32_t)len);
    g_bd_saddr_cyc += (__rt_pmccntr() - __sa0); /* [exp48] */
    g_bd_saddr_n++;
    ...
    g_bd_en_cyc += (__rt_pmccntr() - __en0); /* [exp48] */
    g_bd_en_n++;
    g_bd_mid_cyc += (__rt_pmccntr() - __md0); /* [exp47] mid end */
    g_bd_mid_n++;
    ...
    g_bd_write_n++;
    unsigned long long __tl0 = __rt_pmccntr(); /* [exp47] tail start */
    AIEHLC_LOG(printf(
    ...
    if (AIEHLC_LOG_ENABLED() && g_bd_track_count < BD_TRACK_MAX) {
    ...
    g_bd_tail_cyc += (__rt_pmccntr() - __tl0); /* [exp47] tail end */
    g_bd_tail_n++;
...
/* [bss-skip] Load only PT_LOAD segments that carry file bytes (.text/.data),
 * skipping pure-.bss segments (p_filesz==0). For this GEMM the .bss holds the
 * six ping-pong window buffers (each its own filesz=0 LOAD segment at
 * 0x78000..0x7d000) which are DMA-filled before the core reads them, and the
 * int32 accumulator is a stack local the kernel zeroes explicitly. The driver's
 * default XAie_LoadElfMem() -> XAIE_LOAD_ELF_ALL block-writes zeros across the
 * whole .bss on every tile, which is pure kernel-load overhead here. Gated by
 * the AIEHLC_SKIP_BSS env var so it is opt-in and easy to A/B.
 * WARNING: only safe when no global is read before being written. If a kernel
 * relies on zero-initialized .bss globals, leave AIEHLC_SKIP_BSS unset. */
 ...
 * Compile-time -DAIEHLC_SKIP_BSS_DEFAULT=1 wins (needed on baremetal newlib
 * where getenv() always returns NULL); otherwise fall back to the env var so
 * a Linux host can A/B without a rebuild. */
...
struct_kernel_group __Runtime_load_kernel_group_nt(XAie_DevInst *dev, XAie_LocType *tiles, int n) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (__Runtime_skip_bss_enabled())
            __Runtime_load_elf_mem_skip_bss(dev, s_kernel_tiles[i], s_active_kernel_elf);
        else
            XAie_LoadElfMem(dev, s_kernel_tiles[i], s_active_kernel_elf);
#else
        XAie_LoadElfMem(dev, s_kernel_tiles[i], s_active_kernel_elf);
...
struct_event __Runtime_launch_kernel_group(XAie_DevInst *dev, struct_kernel_group kg) {
     ...
     * moment the cores start running. The first compute tile becomes the probe. */
    /* MM2S BD-finished counters (MEM module, counters 0/1) are set up at device
     * init, but kernel load issues a CoreReset that clears the tile's counters.
     * Re-arm them here (after load, before CoreEnable) so Layer-2 reads are live. */
    if (AIE_DEBUG_HAS_FLAG(g_runtime_debug_level, AIE_DEBUG_FLAG_MM2SBDFINISH_COUNTER)) {
    ...
    while (numPendingBDs > 0) {
        ...
        if (numPendingBDs > 0) {
            if (++iter >= max_iters) {
                ...
                /* Debug: on first timeout, dump klog + lock state for all core tiles */
                static int s_timeout_dumped = 0;
                if (!s_timeout_dumped) {
                    s_timeout_dumped = 1;
                    /* Use partition geometry from dev to avoid invalid tile accesses */
                    int part_start_col = (int)dev->StartCol;
                    int part_num_cols = (int)dev->NumCols;
                    int core_row_start = (int)dev->AieTileRowStart;
                    int core_num_rows = (int)dev->AieTileNumRows;
                    printf("[aie_debug] === TIMEOUT DEBUG DUMP === "
                           "partition startCol=%d numCols=%d coreRowStart=%d coreNumRows=%d\n",
                           part_start_col, part_num_cols, core_row_start, core_num_rows);

                    /* 1) Shim DMA status FIRST (before any core tile access that may crash) */
                    for (int dc = 0; dc < part_num_cols; dc++) {
                        int abs_col = part_start_col + dc;
                        XAie_LocType st = XAie_TileLoc(abs_col, 0);
                        for (int ch = 0; ch <= 1; ch++) {
                            for (int d = 0; d <= 1; d++) {
                                u32 chs = 0;
                                XAie_DmaGetChannelStatus(dev, st, ch, (XAie_DmaDirection)d, &chs);
                                u8 pb = 0;
                                XAie_DmaGetPendingBdCount(dev, st, ch, (XAie_DmaDirection)d, &pb);
                                printf("[aie_debug] shim tile(%d,0) ch=%d dir=%d status=0x%08x pending=%u\n", abs_col,
                                       ch, d, (unsigned)chs, (unsigned)pb);
                            }
                        }
                    }

                    /* 2) Core tile klog + locks (partition-bounded) */
                    for (int dc = 0; dc < part_num_cols; dc++) {
                        int abs_col = part_start_col + dc;
                        for (int dr = core_row_start; dr < core_row_start + core_num_rows; dr++) {
                            XAie_LocType ct = XAie_TileLoc(abs_col, dr);
                            __Runtime_read_kernel_log(dev, ct);
                            /* Read lock values 0-5 */
                            u32 lv[6];
                            for (int li = 0; li < 6; li++) {
                                lv[li] = 0xDEAD;
                                XAie_LockGetValue(dev, ct, XAie_LockInit(li, 0), &lv[li]);
                            }
                            printf("[aie_debug] lock tile(%d,%d): L0=%u L1=%u L2=%u L3=%u L4=%u L5=%u\n", abs_col, dr,
                                   lv[0], lv[1], lv[2], lv[3], lv[4], lv[5]);
                        }
                    }

                    /* 3) Core tile DMA status (partition-bounded) */
                    for (int dc = 0; dc < part_num_cols; dc++) {
                        int abs_col = part_start_col + dc;
                        for (int dr = core_row_start; dr < core_row_start + core_num_rows; dr++) {
                            XAie_LocType ct = XAie_TileLoc(abs_col, dr);
                            for (int ch = 0; ch <= 1; ch++) {
                                for (int d = 0; d <= 1; d++) {
                                    u32 chs = 0;
                                    XAie_DmaGetChannelStatus(dev, ct, ch, (XAie_DmaDirection)d, &chs);
                                    u8 pb = 0;
                                    XAie_DmaGetPendingBdCount(dev, ct, ch, (XAie_DmaDirection)d, &pb);
                                    printf("[aie_debug] core tile(%d,%d) ch=%d dir=%d status=0x%08x pending=%u\n",
                                           abs_col, dr, ch, d, (unsigned)chs, (unsigned)pb);
                                }
                            }
                        }
                    }
                    printf("[aie_debug] === END TIMEOUT DEBUG DUMP ===\n");
                }
                return;
            }
...
AieRC __Runtime_core_perf_setup(XAie_DevInst *dev, XAie_LocType tile) {
    ...
    if (rc != XAIE_OK)
        return rc;
    /* Counter 2: stream stall cycles — self-counting */
    ...
    if (rc != XAIE_OK)
        return rc;
    /* Counter 3: lock stall cycles — self-counting */
