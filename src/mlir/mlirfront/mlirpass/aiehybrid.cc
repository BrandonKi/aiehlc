	...
	for(auto *op : ops) {
        ...
        for (mlir::Value value : kop.getArguments()) {
            ...
            if (isPingPongMode) {
                ...
                uint32_t pongOffset = window.getPingaddr() + single_buf_bytes;

                std::string wping = wname + "_ping";
                std::string wpong = wname + "_pong";
                uint32_t ping_chess = 0x78000 + (uint32_t)window.getPingaddr();
                bcf.addsymbols(wping, ping_chess);
                bcf.addsymbols(wpong, 0x78000 + pongOffset);

                kfuncparams.push_back(Buffer(direct, wping, wpong, window.getPingaddr(), pongOffset,
                                             /*acquireLock=*/autoLockId,
                                             /*releaseLock=*/autoLockId + 1));
                kfuncparam_chess_addrs.push_back(ping_chess);
                autoLockId += 2;
                if (window.getPingaddr() > 0)
                    max_window_size = std::max(max_window_size, (uint32_t)window.getPingaddr());
            }
        }
...

        {
            ...
            if (dm_offsets.is_open()) {
                ...
                dm_offsets << "// Sim uses the same BCF (aieml.bcf fallback in script/sim/Makefile).\n";
