...
//   A (im2col matrix): [M, K] = [OH*OW, KH*KW*C] = [12544, 147]
//   B (filter matrix): [K, N] = [KH*KW*C, F]      = [147, 64]
//   C (output):        [M, N] = [OH*OW, F]         = [12544, 64]
#define M (OUTPUT_H * OUTPUT_W)           // 12544
#define K (KERNEL_H * KERNEL_W * INPUT_C) // 147
...
static void host_im2col(const int8_t *input, int8_t *im2col_out) {
    ...
    for (int oh = 0; oh < OUTPUT_H; oh++) {
        for (int ow = 0; ow < OUTPUT_W; ow++) {
            // Extract one patch: KH x KW x C elements
            for (int kh = 0; kh < KERNEL_H; kh++) {
                for (int kw = 0; kw < KERNEL_W; kw++) {
                    for (int c = 0; c < INPUT_C; c++) {
                        int ih = oh * STRIDE + kh;
                        int iw = ow * STRIDE + kw;
                        im2col_out[idx++] = input[(ih * INPUT_W + iw) * INPUT_C + c];
                    }
                }
            }
        }
                ...
                for (int kh = 0; kh < KERNEL_H; kh++) {
                    for (int kw = 0; kw < KERNEL_W; kw++) {
                        for (int c = 0; c < INPUT_C; c++) {
                            int ih = oh * STRIDE + kh;
                            int iw = ow * STRIDE + kw;
                            int8_t iv = input[(ih * INPUT_W + iw) * INPUT_C + c];
                            int8_t fv = filter[((kh * KERNEL_W + kw) * INPUT_C + c) * NUM_FILTERS + f];
...
static int verify_conv2d(const int8_t *input, const int8_t *filter, const int8_t *output_aie) {
    ...
    for (int h = 0; h < INPUT_H; h++) {
        ...
        for (int w = 0; w < INPUT_W; w++) {
            printf("%4d", input[h * INPUT_W * INPUT_C + w * INPUT_C]);
            if (w < INPUT_W - 1)
                printf(",");
        }
        printf("]\n");
    ...
    for (int kh = 0; kh < KERNEL_H; kh++) {
        printf("  [");
        for (int kw = 0; kw < KERNEL_W; kw++) {
            printf("%4d", filter[(kh * KERNEL_W + kw) * INPUT_C * NUM_FILTERS]);
            if (kw < KERNEL_W - 1)
                printf(",");
        }
        printf("]\n");
